...
//// ŻEBY TEN TEST PRZESZEDŁ TRZEBA PODMIENIĆ RETURN W 'DEFAULT_PROBABILITY_GENERATOR()' W 'HELPERS.CPP' - TYLKO STARA WERSJA ADD_RECEIVER()
...
//// ŻEBY TEN TEST PRZESZEDŁ TRZEBA PODMIENIĆ FUNKCJĘ W 'CHOOSE_RECEIVER()' W 'NODES.CPP'
...
class PackageSenderFixture : public PackageSender {
    // Nie sposób w teście wykorzystać prywatnej metody `PackageSender::push_package()`,
    // dlatego do celów testowych stworzona została implementacja zawierająca
    // metodę `push_package()` w sekcji publicznej.
public:
    PackageSenderFixture() {PackageSender();};
...
TEST(PackageSenderTest, SendPackage) {
    ...
    // wywołana dwukrotnie, z dowolnym argumentem (symbol `_`).                     //// NA PEWNO DWUKROTNIE???

//    EXPECT_CALL(mock_receiver, receive_package(_)).Times(1);                      //// NIE POTRAFIĘ TEGO ZMOCKOWAĆ
    ...
    sender.send_package();                                                          //// TU WYKONUJE SIĘ RECEIVE_PACKAGE()
